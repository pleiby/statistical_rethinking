...
See [`rethinking` github page](https://github.com/rmcelreath/rethinking)
...
The first formula in the list is the probability of the outcome `y` (likelihood); the second is the prior for `mu`; the third is the prior for `sigma`.
...
The same formula list can be compiled into a Stan (mc-stan.org) model using one
of two tools: `ulam` or `map2stan`. For simple models, they are identical.
`ulam` is the newer tool that allows for much more flexibility, including
explicit variable types and custom distributions.
`map2stan` is the original tool from the first edition of the package and textbook.
Going forward, new features will be added to `ulam`.
...
The chain runs automatically, provided `rstan` is installed.
Chain diagnostics are displayed in the `precis(fit_stan)` output:
...
The Stan code can be accessed by using 
...
`sim` is used to simulate posterior predictive distributions, simulating
outcomes over samples from the posterior distribution of parameters.
`sim` can also be used to simulate prior predictives.

See `?link` and `?sim` for details.

`postcheck` automatically computes posterior predictive (retrodictive?) checks.
It merely uses link and sim.

### Multilevel model formulas

While `quap` is limited to fixed effects models for the most part, 
`ulam` can specify multilevel models, even quite complex ones.
For example, a simple varying intercepts model looks like:
